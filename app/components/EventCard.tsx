...
function ModeBadge({ mode }: { mode?: "local" | "remote" }) {
  ...
  return (
    <span className="rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-700">
      {mode === "remote" ? "TEE Remote" : "TEE Enclave"}
    </span>
  );
}
...
export function EventCard({
  ...
  if (ev.type === "log") {
    return (
      <div className="event-enter flex items-center gap-2 py-1 pl-3 text-sm text-slate-400">
  ...
  if (ev.type === "serendipity") {
    return (
      <div className="event-enter rounded-2xl border-2 border-amber-300 bg-amber-50 p-5 shadow-sm">
        <div className="mb-2 flex items-center gap-2 text-base font-bold text-amber-700">
          <svg className="h-5 w-5 text-amber-500" fill="currentColor" viewBox="0 0 20 20">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
          {ev.title}
          {ev.serendipityScore != null && (
            <span className="rounded-full bg-amber-200 px-3 py-0.5 text-xs font-bold text-amber-800">
              Serendipity {Math.round(ev.serendipityScore * 100)}%
            </span>
          )}
        </div>
        <p className="text-base leading-relaxed text-amber-800">{ev.detail}</p>
      </div>
    );
  }
  ...
  if (ev.type === "matching") {
    return (
      <div className="event-enter rounded-2xl border border-blue-200 bg-blue-50 p-5">
        <div className="mb-2 text-base font-bold text-blue-700">{ev.title}</div>
  ...
  if (ev.type === "final") {
    return (
      <div className="event-enter rounded-2xl border-2 border-blue-400 bg-gradient-to-br from-blue-50 to-white p-6 shadow-md">
        <div className="mb-3 text-base font-bold tracking-wide text-blue-700">
          {ev.title}
        </div>
  ...
  if (ev.type === "critique") {
    return (
      <div className="event-enter rounded-2xl border border-purple-200 bg-purple-50 p-5">
        <div className="mb-2 text-base font-bold text-purple-700">{ev.title}</div>
  ...
  if (ev.type === "draft") {
    return (
      <div className="event-enter rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5">
        <div
          className="mb-2 flex items-center gap-2 text-sm font-bold uppercase tracking-wide"
          style={{ color }}
        >
          <span className="h-3 w-3 rounded-full" style={{ background: color }} />
          {label} — Internal Draft (TEE Enclave)
  ...
  if (ev.type === "wall_flag") {
    return (
      <div className="event-enter rounded-2xl border border-red-200 bg-white p-4">
  ...
  if (ev.type === "wall_block") {
    return (
      <div className="event-enter rounded-2xl border-2 border-red-300 bg-red-50 p-5">
        <div className="mb-2 flex items-center gap-2 text-base font-bold text-red-700">
          BLOCKED — {ev.title}
          <ModeBadge mode={ev.mode} />
        </div>
  ...
  return (
    <div className="event-enter rounded-2xl border border-emerald-200 bg-white p-4">
      <div
